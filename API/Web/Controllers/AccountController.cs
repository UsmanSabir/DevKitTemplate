...
namespace Api.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous, HttpPost("~/connect/token"), Produces("application/json")]
        ...
        public async Task<IActionResult> Authorize(OpenIdConnectRequest request)
        {
            ...
            if (info == null)
            {
                ...
                return BadRequest("error");// Render(ExternalLoginStatus.Error);
            }
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(email))
                {
                    ...
                    {
                        return BadRequest("error");// BadRequest(new ApiError("Email already exists"));
                    }
                }
                else
                {
                    // External account doesn't have a local account so ask to create one
                    return Content("create account");// Render(ExternalLoginStatus.CreateAccount);
                }

            }


            return BadRequest("ExternalLoginStatus error");// Render(ExternalLoginStatus.Error);
            ...
            AuthenticationProperties properties = null)
        {
            ...
            //                    if (claim.Type == OpenIdConnectConstants.Claims.Role && ticket.HasScope(OpenIddictConstants.Claims.Roles))
            //                    {
            //                        claim.SetDestinations(OpenIdConnectConstants.Destinations.AccessToken,
            //                            OpenIdConnectConstants.Destinations.IdentityToken);
            //                    }
